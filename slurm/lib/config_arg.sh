#!/bin/bash
# `--config` handling for slurm/train_diffusion.sbatch, which has its own job body.
#
# Every other train_*.sbatch has moved onto slurm/lib/train_common.sh, which does this parsing
# itself; this file stays only for the diffusion job and goes when that job migrates too.
#
